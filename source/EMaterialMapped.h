	...
	public:
		...
		RGBColor Shade(const VertexOutput& v, const FVector3& lightDirection, const FVector3& viewDirection, const FVector3& normal) const override
		{
			...
			if (m_pSpecularMap != nullptr && m_pGlossinessMap != nullptr)
				tempColor += BRDF::Phong(m_pSpecularMap->Sample(v.uv), m_pGlossinessMap->SampleF(v.uv) * m_Shininess, lightDirection, viewDirection, normal);
			return tempColor;
		}
		...
		//Getters
		/*D3D*/
		FVector3 GetMappedNormal(const VertexOutput& v) const override
		{
			if (m_pNormalMap == nullptr)
				return v.normal;
